...
router.get('/overview', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Query 1: Contar total e ativos de veículos
  const veiculos = await pool.query('SELECT COUNT(*) as total FROM veiculo')
  const veiculosAtivos = await pool.query('SELECT COUNT(*) as ativos FROM veiculo WHERE status = $1', ['ativo'])
  
  // Query 2: Contar total e ativos de motoristas
  const motoristas = await pool.query('SELECT COUNT(*) as total FROM motorista')
  const motoristasAtivos = await pool.query('SELECT COUNT(*) as ativos FROM motorista WHERE status = $1', ['ativo'])
  
  // Query 3: Contar cidades
  const cidades = await pool.query('SELECT COUNT(*) as total FROM cidade')

  // Query 4: Contar viagens totais no período
  const viagensTotal = await pool.query(
    'SELECT COUNT(*) as total FROM viagem WHERE data_saida >= CURRENT_DATE - ($1::text || \' months\')::interval',
    [meses]
  )
  
  // Query 5: Contar viagens em andamento
  const viagensAndamento = await pool.query(
    'SELECT COUNT(*) as em_andamento FROM viagem WHERE status_viagem = $1 AND data_saida >= CURRENT_DATE - ($2::text || \' months\')::interval',
    ['em_andamento', meses]
  )
  
  // Query 6: Contar viagens finalizadas
  const viagensFinalizadas = await pool.query(
    'SELECT COUNT(*) as finalizadas FROM viagem WHERE status_viagem = $1 AND data_saida >= CURRENT_DATE - ($2::text || \' months\')::interval',
    ['finalizada', meses]
  )

  // Query 7: Calcular KM total das viagens finalizadas
  const kmViagens = await pool.query(
    'SELECT SUM(km_final - km_inicial) as km_total FROM viagem WHERE km_final IS NOT NULL AND data_saida >= CURRENT_DATE - ($1::text || \' months\')::interval',
    [meses]
  )

  // Query 8: Somar custos de combustível
  const custoCombustivel = await pool.query(
    'SELECT SUM(valor_total) as total FROM abastecimento WHERE data_abast >= CURRENT_DATE - ($1::text || \' months\')::interval',
    [meses]
  )
  
  // Query 9: Somar custos de manutenção concluída
  const custoManutencao = await pool.query(
    'SELECT SUM(valor) as total FROM manutencao WHERE concluida = true AND data_man >= CURRENT_DATE - ($1::text || \' months\')::interval',
    [meses]
  )

  // Query 10: Contar veículos em viagem
  const veiculosEmViagem = await pool.query('SELECT COUNT(*) as em_viagem FROM veiculo WHERE status = $1', ['em_viagem'])
  
  // Query 11: Contar veículos em manutenção
  const veiculosManutencao = await pool.query('SELECT COUNT(*) as manutencao FROM veiculo WHERE status = $1', ['manutencao'])

  // Query 12: Contar motoristas em viagem
  const motoristasEmViagem = await pool.query(
    'SELECT COUNT(DISTINCT cpf_motorista) as em_viagem FROM viagem WHERE status_viagem = $1',
    ['em_andamento']
  )

  // Calcular totais em JavaScript
  const totalCombustivel = parseFloat(custoCombustivel.rows[0].total || 0)
  const totalManutencao = parseFloat(custoManutencao.rows[0].total || 0)
  const custoTotal = totalCombustivel + totalManutencao
  const kmTotal = parseInt(kmViagens.rows[0].km_total || 0, 10)
...
router.get('/frota-completo', asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    totalizadores: {
      ...
      total_viagens: veiculos.reduce((acc, v) => acc + v.total_viagens, 0)
    }
  })
}))
...
router.get('/motoristas-completo', asyncHandler(async (req: Request, res: Response) => {
  ...
  const motoristas = motoristasDetalhado.rows.map(m => {
    ...
    const horasValidas = (m.horas_array || []).filter((h: any) => h !== null && h !== undefined)
    const horasMediaViagem = horasValidas.length > 0 
      ? horasValidas.reduce((a: number, b: any) => a + b, 0) / horasValidas.length 
      : 0
...
router.get('/manutencao-critica', asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    veiculos: manutencoesPorVeiculo.rows.map(v => {
      ...
      const datas = (v.datas_manutencoes || []).filter((d: any) => d !== null && d !== undefined)
      if (datas.length > 1) {
        let totalDias = 0
...
router.get('/rotas-analise', asyncHandler(async (req: Request, res: Response) => {
  ...
  const rotas = rotasDetalhadas.rows.map(r => {
    ...
    // Calcular média de horas a partir do array
    const horasValidas = (r.horas_array || []).filter((h: any) => h !== null && h !== undefined)
